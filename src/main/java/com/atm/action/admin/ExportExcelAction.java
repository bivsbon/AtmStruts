...
public class ExportExcelAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		List<TransactionExtended> exportList = (List<TransactionExtended>) session.getAttribute("adminExportList");
		int rowNum = 1;
		
		for (TransactionExtended trans : exportList) {
			Row row = sh.createRow(rowNum++);
			row.createCell(0).setCellValue(trans.getType().toString());
			row.createCell(1).setCellValue(trans.getDate());
			row.createCell(2).setCellValue(trans.getTime());
		...
		if (!folder.exists()) {
			if (!folder.mkdir()) {
				System.out.println("Cant' create excel folder");
			}
		}
